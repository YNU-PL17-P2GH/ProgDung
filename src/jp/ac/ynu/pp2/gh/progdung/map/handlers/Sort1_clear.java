...
public class Sort1_clear extends MapHandlerBase{

	private MapClearObject pc;

	public Sort1_clear(int player_x, int player_y, DIRECTION player_d, DungeonPlay play) {
		super("sort1_clear", player_x, player_y, player_d, play);
	}
	...
	public void onPlayerInteract(MapObject pObject) {
		if(pObject instanceof MapClearObject){
			if(!callback.getSaveData().getBoolean("Sort1001")){
				showHint("<html>なんと鍵をみつけた!!!</html>", true);
				pc.setMahouzinOnFlag(true);
				callback.getSaveData().setTaken("Sort1001");
			}
		}
	}
	...
	public void onMapLoad(){
		for (MapObject tObject : theObj) {
			if (tObject instanceof MapClearObject) {
				((MapClearObject)tObject).setMahouzinOnFlag(callback.getSaveData().getBoolean("Sort1001"));
