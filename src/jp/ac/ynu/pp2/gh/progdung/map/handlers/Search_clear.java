...
public class Search_clear  extends MapHandlerBase {

	private MapClearObject pc;

	public Search_clear(int player_x, int player_y, DIRECTION player_d, DungeonPlay play) {
		super("search_clear", player_x, player_y, player_d, play);
	}
	...
	public void onPlayerInteract(MapObject pObject) {
		if(pObject instanceof MapClearObject){
			if(!callback.getSaveData().getBoolean("Search002")){
				showHint("<html>なんと鍵をみつけた!!!</html>", true);
				pc.setMahouzinOnFlag(true);
				callback.getSaveData().setTaken("Search002");
			}
		}
	}
