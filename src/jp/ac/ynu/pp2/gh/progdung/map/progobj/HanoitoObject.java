	...
	public HanoitoObject(MapHandlerBase pHandler, int bx, int by, String pObjName, RpgMap pMap) {
		...
		try {
			for (int i = 0; i < towerImgs.length; i++) {
				String s = "FirstRpg/media/map/hanoi/hanoi" + i +".png";
				towerImgs[i] = ImageIO.read(getClass().getClassLoader().getResource("FirstRpg/media/map/hanoi/hanoi" + (towerImgs.length - i - 1) + ".png"));
			}
		} catch (IOException e) {
