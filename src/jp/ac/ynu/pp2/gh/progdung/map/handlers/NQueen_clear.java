...
public class NQueen_clear  extends MapHandlerBase {

	private MapClearObject pc;

	public NQueen_clear(int player_x, int player_y, DIRECTION player_d, DungeonPlay play) {
		super("nqueen_clear", player_x, player_y, player_d, play);
	}
	...
	public void onPlayerInteract(MapObject pObject) {
		if(pObject instanceof MapClearObject){
			if(!callback.getSaveData().getBoolean("NQueen001")){
				showHint("<html>なんと鍵をみつけた!!!</html>", true);
				pc.setMahouzinOnFlag(true);
				callback.getSaveData().setTaken("NQueen001");
			}
		}
	}
